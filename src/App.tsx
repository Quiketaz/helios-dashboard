...
const App = () => {
  ...
  if (loading) return <div className="min-h-screen bg-black flex items-center justify-center text-teal-500 font-black italic tracking-widest uppercase">SYNCING HELIOS...</div>;
  ...
  return (
    <div className="min-h-screen bg-black text-zinc-200 font-sans flex flex-col md:flex-row">
      <nav className="w-full md:w-20 lg:w-64 bg-zinc-950 border-r border-zinc-900 p-6 flex flex-col gap-2">
        <div className="hidden lg:block mb-8 text-2xl font-black italic text-white uppercase tracking-tighter">HELIOS<span className="text-teal-500">.</span></div>
...
const NavBtn = ({ id, active, set, icon, label }: any) => (
  <button onClick={() => set(id)} className={`flex items-center gap-4 p-3 rounded-xl transition-all w-full ${active === id ? 'bg-teal-500 text-black font-black' : 'text-zinc-500 hover:text-white'}`}>
